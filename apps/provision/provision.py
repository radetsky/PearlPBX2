...
class GrandstreamGXPGenerator(PhoneConfigGenerator):
    ...
    def _generate_config_text(
        self, name: str, secret: str, sipserver: str, **kwargs
    ) -> str:
        ...
        sorted_params = sorted(config_params.items(), key=lambda x: int(x[0][1:]))
    ...
    def generate_config(self, **kwargs) -> bytes:
        ...
        if (
            ...
            or not mac_address.islower()
            or len(mac_address) != 12
            or not all(c in "0123456789abcdef" for c in mac_address)
        ):
            raise ValueError(
                "mac_address must be a 12-character lowercase hex string (0-9, a-f) without separators"
            )
        ...
        if not all([mac_address, name, secret, sipserver]):
            raise ValueError(
                "mac_address, name, secret, and sipserver are required parameters"
            )
        ...
        filtered_kwargs = {
            ...
            if k not in ["name", "secret", "sipserver", "mac_address"]
        }
...
class Provisioning:
    ...
    def generate_grandstream_config(
        self, mac_address: str, name: str, secret: str, sipserver: str, **kwargs
    ) -> bytes:
    ...
    def generate_cisco_config(
        self, name: str, secret: str, sipserver: str, model: str = "SPA504G", **kwargs
    ) -> bytes:
        """Generate configuration for Cisco SPA phones"""
        return self.generators["cisco"].generate_config(
            name=name, secret=secret, sipserver=sipserver, model=model, **kwargs
        )
