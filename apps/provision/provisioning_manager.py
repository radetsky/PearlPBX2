...
from apps.provision.provision import Provisioning, GrandstreamGXPGenerator, CiscoSPAGenerator
...
class PhoneProvisioningManager:
    ...
    def generate_config_for_device(self, device):
        ...
        if device.telephone_type not in self.phone_mapping:
            raise ValueError(f"Phone type {device.telephone_type} is not supported for provisioning")
