...
# Extension pattern used by the ConfBridge conference landing extension created by
# core.migrations.0079_create_conference_dialplan_context (room number = ${EXTEN}).
# Room numbers generated by apps.api.serializers.ConferenceSerializer must match it.
CONFERENCE_ROOM_EXTENSION_PATTERN = "_X."
...
def make_confbridge_conf():
    """
    Minimal confbridge.conf: default bridge/user profiles only.
    Conference rooms are dynamic (created on first join, destroyed on last leave) —
    there is no per-room configuration in this version.
    """
    plaintext = AUTO_GENERATED_HEADER
    plaintext += "; === Use PearlPBX admin panel! ===\n"
    plaintext += GENERAL_SECTION
    ...
    plaintext += "type=user\n"
    plaintext += "\n[default_bridge]\n"
    plaintext += "type=bridge\n"
    plaintext += "max_members=0\n"
    return plaintext
