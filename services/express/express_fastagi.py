...
class ULineManager:
    ...
    def allocate_uline(self, cdr_start: str, cdr_uniqueid: str, channel: str) -> Optional[int]:
        ...
        if cdr_uniqueid in self.uniqueid_to_uline:
            ...
            logger.info(f"Call {cdr_uniqueid} already has ULINE {existing_uline}, returning existing")
        ...
        for uline in range(ExpressConfig.ULINE_MIN, ExpressConfig.ULINE_MAX + 1):
            if uline not in self.ulines:
                ...
                logger.info(f"Allocated ULINE {uline} for call {cdr_uniqueid} (channel: {channel})")
                return uline
    ...
    def update_uline(self, cdr_uniqueid: str, new_channel: str) -> Optional[int]:
        ...
        logger.info(f"Updated ULINE {uline} for call {cdr_uniqueid}: {old_channel} -> {new_channel}")
        return uline
...
def main():
    ...
    try:
        ...
        # Start server
        reactor.listenTCP(
            ExpressConfig.FASTAGI_PORT,
            factory,
            interface=ExpressConfig.FASTAGI_HOST
        )
        
        logger.info(f"FastAGI server started on {ExpressConfig.FASTAGI_HOST}:{ExpressConfig.FASTAGI_PORT}")
