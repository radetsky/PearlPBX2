...
- Docker-розгортання: `docker-compose.yml` (pearlpbx2, asterisk, postgres, redis, fastagi, dashboard-listener, callback)
...
Роль `nginx` автоматично отримує сертифікат, у такому порядку пріоритету:
Let's Encrypt для `pearlpbx2_domain`, якщо задано (`--domain` у `install.sh`),
інакше — сертифікат Let's Encrypt на публічну IPv4 хоста (термін дії 6 днів),
інакше — самопідписаний сертифікат. Результат записується у
`/etc/PearlPBX/tls-mode` (`letsencrypt` або `selfsigned`).

- **Файли сертифіката**: `/etc/ssl/pearlpbx2/fullchain.pem` та `privkey.pem` —
  завжди симлінки, або на `/etc/letsencrypt/live/pearlpbx2/`, або на пару
  `selfsigned-*.pem` у тій самій директорії. Директиви `ssl_certificate` в
  nginx ніколи не змінюються, тож перемикання джерела сертифіката не потребує
  повторного шаблонування vhost.
- **Додавання домену пізніше**: повторно запустіть
  `sudo ./install.sh --domain <fqdn> --email <you>`. `update.sh` не чіпає
  nginx чи TLS — для цього використовуйте `install.sh`.
- **Продовження терміну дії**: systemd-таймер (`pearlpbx2-certbot-renew.timer`,
  кожні 4 години) запускає `certbot renew`. Перевірити:
  `systemctl list-timers pearlpbx2-certbot-renew` або
  `sudo /opt/certbot/bin/certbot certificates`.
- **Примусовий self-signed**: встановіть `acme_enabled: false` у
  `ansible/group_vars/all.yml` і повторно запустіть `install.sh`.
- **Немає публічного порту 80 / хост без мережі**: ACME завершується без
  помилки, а роль переходить на self-signed; причина друкується в лозі
  інсталяції (`/var/log/pearlpbx2-install.log`).
...
5. Видалення SIP-користувача автоматично прибирає й його запис(и) `QueueMember`, тож видалений extension більше не лишається в черзі "мертвим" членом. Команда `python manage.py cleanup_orphan_queue_members` прибирає осиротілі рядки, що лишились до появи цього автоматичного очищення.
...
Веб-хуки дозволяють автоматично надсилати CRM-системі JSON POST-запити про події дзвінків: два незалежні ланцюги — вхідний (`call.incoming` → `call.answered`/`call.missed` → `call.ended`, дзвінки ззовні або через транк) і вихідний (`call.outgoing` → `call.outgoing_answered` → `call.outgoing_ended`, дзвінки, ініційовані SIP-користувачем, ніколи транком). Реалізовано в `apps/webhooks/` — доставку обробляє Dashboard Listener на основі подій AMI.
...
| **Send Ended** | Надсилати подію на завершення вхідного дзвінка (потребує ввімкненого Send Incoming, інакше дзвінок не буде "анонсовано") |
...
| **Send Outgoing** | Надсилати подію, коли SIP-користувач ініціює вихідний дзвінок (потребує вибору хоча б однієї Routing table; ніколи не спрацьовує для транка) |
| **Send Outgoing Answered** | Надсилати подію, коли викликана сторона підняла слухавку (потребує ввімкненого Send Outgoing) |
| **Send Outgoing Ended** | Надсилати подію на завершення вихідного дзвінка (потребує ввімкненого Send Outgoing) |
| **Contexts** | Контексти діалплану, вхідні дзвінки в яких запускають вхідний ланцюг (Send Incoming тощо) |
| **Routing tables** | Routing tables SIP-користувачів, чиї вихідні дзвінки запускають вихідний ланцюг (Send Outgoing тощо) |
| **Queues** | Черги, приєднання до яких запускає події вхідного ланцюга, пов'язані з чергами |
...
**Примітка:** якщо для веб-хука не вибрано жодного контексту, routing table чи черги, форма адміна вимагає вказати принаймні одне з них (інакше незрозуміло, які дзвінки мають запускати доставку). Вхідні події (Send Incoming тощо) додатково вимагають Contexts або Queues; вихідні події (Send Outgoing тощо) — Routing tables.
...
*Документ створено для PearlPBX2 v2.7.2. Інтерфейс системи та шляхи можуть відрізнятися залежно від конфігурації.*
